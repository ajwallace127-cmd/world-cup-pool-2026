...
# Teams that lost via ET or penalties in a knockout game.
# Use this if ESPN's status fields don't correctly flag a game as ET/PK
# (loser should get 100 pts, not 0). Add the LOSING team's name (as it
# appears in TIERS or TEAM_ALIASES — normalization is applied automatically).
MANUAL_ET_PK_LOSERS: set = {
    # "South Africa",  # example: lost to Canada on penalties
}
...
def fetch_results() -> tuple[dict, set, set, set, dict, dict]:
    ...
    """
    team_results     = defaultdict(lambda: {"wins": 0, "ties": 0, "losses": 0, "et_losses": 0})
    group_game_count = defaultdict(int)
    advanced         = set()
    knocked_out      = set()
    champion         = set()
    event_ids        = []
    third_place_ids  = set()   # Survivor rules: 3rd-place match excluded from scoring
    ...
    while current <= min(today, TOURNAMENT_END):
        ...
        try:
            ...
            for event in data.get("events", []):
                ...
                # Detect ET/PK finish: loser gets 100 pts; regulation loss = 0 pts.
                # Check every ESPN status subfield across both event-level and
                # competition-level status objects — ESPN is inconsistent about
                # which field it populates for different tournaments/rounds.
                status_name   = status.get("name", "").upper()
                status_desc   = status.get("description", "").lower()
                status_detail = status.get("detail", "").lower()
                status_short  = status.get("shortDetail", "").upper()

                # Competition-level status (sometimes differs from event-level)
                comp_stype  = comp.get("status", {}).get("type", {})
                comp_sname  = comp_stype.get("name", "").upper()
                comp_sdesc  = comp_stype.get("description", "").lower()
                comp_sdet   = comp_stype.get("detail", "").lower()
                comp_sshort = comp_stype.get("shortDetail", "").upper()

                _ET_TOKENS  = ("AET", "EXTRA")
                _PK_TOKENS  = ("PEN", "PSO", "SHOOT", "PENALT")
                _ALL_TOKENS = _ET_TOKENS + _PK_TOKENS

                def _has(text: str, tokens) -> bool:
                    return any(t in text for t in tokens)

                is_et_pk = (
                    _has(status_name,   _ALL_TOKENS) or
                    _has(status_desc,   ("extra time", "aet", "penalt", "shootout", "pso")) or
                    _has(status_detail, ("extra time", "aet", "penalt", "shootout", "pso")) or
                    _has(status_short,  _ALL_TOKENS) or
                    _has(comp_sname,    _ALL_TOKENS) or
                    _has(comp_sdesc,    ("extra time", "aet", "penalt", "shootout", "pso")) or
                    _has(comp_sdet,     ("extra time", "aet", "penalt", "shootout", "pso")) or
                    _has(comp_sshort,   _ALL_TOKENS)
                )
                ...
                # Date-based override: group stage ended June 27, 2026.
                # Any game on or after June 28 that ESPN mis-labels as "GS"
                # must actually be a knockout round — use the schedule dates.
                if round_cat == "GS" and current >= date(2026, 6, 28):
                    if   current >= date(2026, 7, 19): round_cat = "F"
                    elif current >= date(2026, 7, 18): round_cat = "3P"
                    elif current >= date(2026, 7, 14): round_cat = "SF"
                    elif current >= date(2026, 7, 9):  round_cat = "QF"
                    elif current >= date(2026, 7, 4):  round_cat = "R16"
                    else:                               round_cat = "R32"

                is_group  = round_cat == "GS"
                is_final  = round_cat == "F"
                # Survivor format: once you lose, you're eliminated. Both
                # 3rd-place-match teams already lost their semifinals, so
                # this game awards NO team points and NO goal-scorer goals.
                # It is recorded for bracket display only.
                is_third  = round_cat == "3P"
                ...
                # Manual ET/PK override: if either team is in MANUAL_ET_PK_LOSERS,
                # treat the game as an ET/PK finish regardless of ESPN status fields.
                if not is_et_pk:
                    if (_normalize(n0) in {_normalize(t) for t in MANUAL_ET_PK_LOSERS} or
                            _normalize(n1) in {_normalize(t) for t in MANUAL_ET_PK_LOSERS}):
                        is_et_pk = True

                if is_et_pk and not is_group:
                    print(f"  [ET/PK] {n0} vs {n1} on {current}")

                eid = event.get("id", "")
                event_ids.append(eid)
                if is_third and eid:
                    # Exclude this game's goals from scorer tallies
                    third_place_ids.add(eid)
                ...
                winner_name = ""
                if is_third:
                    # Bracket display only — no W/L credit, no elimination
                    # changes (both teams were eliminated in the semifinals).
                    if w0 and not w1:
                        winner_name = n0
                    elif w1 and not w0:
                        winner_name = n1
                    ...
                    if is_group:
                        ...
                        # ET/PK loss = 100 pts; regulation knockout loss = 0 pts
                        if is_et_pk:
                            team_results[n1]["et_losses"] += 1
                        else:
                            team_results[n1]["losses"] += 1
    ...
    if third_place_ids:
        print(f"  Survivor rules: excluding {len(third_place_ids)} 3rd-place match(es) from team points & scorer goals")
    scorer_goals = fetch_goal_scorers(
        [eid for eid in event_ids if eid and eid not in third_place_ids]
    )
...
def match_team(pick: str, team_results: dict, advanced: set,
    ...
    _empty = {
        "wins": 0, "ties": 0, "losses": 0, "et_losses": 0,
    ...
    # advanced, champion, knocked_out are all NORMALIZED string sets.
    # Never include them in all_names — they create phantom lowercase entries
    # that match pick_norm but have no team_results data (e.g. "norway" != "Norway").
    # Check all three via pick_norm directly instead.
    all_names = set(team_results.keys())

    # advanced, champion, knocked_out are all normalized sets — use pick_norm directly.
    for name in all_names:
        if _normalize(name) == pick_norm:
            r   = team_results.get(name, {"wins": 0, "ties": 0, "losses": 0, "et_losses": 0})
            pts = r["wins"] * WIN_PTS + (r["ties"] + r.get("et_losses", 0)) * TIE_PTS
            return {
                "wins": r["wins"], "ties": r["ties"], "losses": r["losses"],
                "et_losses": r.get("et_losses", 0),
                "advanced":    pick_norm in advanced,
                "champion":    pick_norm in champion,
                "knocked_out": pick_norm in knocked_out,
                "matched_name": name,
                "has_played": r["wins"] + r["ties"] + r["losses"] + r.get("et_losses", 0) > 0 or pick_norm in advanced,
                "pts_so_far": pts,
            }
    ...
    if best_name:
        r   = team_results.get(best_name, {"wins": 0, "ties": 0, "losses": 0, "et_losses": 0})
        pts = r["wins"] * WIN_PTS + (r["ties"] + r.get("et_losses", 0)) * TIE_PTS
        return {
            "wins": r["wins"], "ties": r["ties"], "losses": r["losses"],
            "et_losses": r.get("et_losses", 0),
...
def calculate_scores(entries: list, team_results: dict, advanced: set,
    ...
    for entry in entries:
        ...
        games_played = sum(
            max(d["wins"] + d["ties"] + d["losses"] + d.get("et_losses", 0), 3)
            if (d["advanced"] or d["knocked_out"])
            else (d["wins"] + d["ties"] + d["losses"] + d.get("et_losses", 0))
            for d in detail
        )
        standings.append({
...
def _chip_html(d: dict) -> str:
    ...
    played = d["wins"] + d["ties"] + d["losses"] + d.get("et_losses", 0) > 0
    if d["knocked_out"]:
        css, icon = "chip-out", "✗"
    elif d["champion"]:
...
def generate_html(standings: list, team_results: dict, advanced: set,
    ...
    for tier, teams in TIER_DEFS.items():
        for team in teams:
            ...
            et_losses = r.get("et_losses", 0)
            game_pts  = wins * WIN_PTS + (ties + et_losses) * TIE_PTS
            ...
            played = wins + ties + losses + et_losses > 0
            ...
            all_teams_data.append({
                ...
                "wins": wins, "ties": ties, "losses": losses, "et_losses": et_losses,
    ...
    for t in all_teams_data:
        ...
        tier_pill = (f'<span class="tier-pill" '
                     ...
                     f'{t["tier"]}</span>')
        gp           = t["wins"] + t["ties"] + t["losses"] + t.get("et_losses", 0)
        et_losses    = t.get("et_losses", 0)
        record       = f'{t["wins"]}W-{t["ties"]}D-{t["losses"]}L' + (f'-{et_losses}ET' if et_losses else '')
...
<head>
  ...
  <style>
    ...
    *, body {{ font-family:'Inter',sans-serif; color:var(--text); }}
    ...
    .sr-section {{ flex:1; min-width:220px; }}
    .sr-heading {{
      font-size:.72rem; font-weight:700; text-transform:uppercase;
      letter-spacing:.5px; color:var(--pitch); margin-bottom:8px;
    }}
    .sr-row {{ display:flex; align-items:center; gap:8px; margin-bottom:5px; font-size:.82rem; }}
    .sr-pts {{
      min-width:44px; text-align:right; font-weight:700; font-size:.8rem;
      padding:2px 6px; border-radius:4px;
    }}
    .sr-win   {{ background:#DCFCE7; color:#166534; }}
    .sr-draw  {{ background:#FEF9C3; color:#854D0E; }}
    .sr-loss  {{ background:#F1F5F9; color:#64748B; }}
    .sr-bonus {{ background:#EDE9FE; color:#5B21B6; }}
    .sr-desc  {{ color:#475569; }}
    .sr-desc em {{ font-style:normal; color:#94A3B8; }}
    .sr-divider {{
      width:1px; background:#E2E8F0; margin:4px 0;
    }}
    @media (max-width:600px) {{
      .sr-divider {{ width:100%; height:1px; }}
    }}
...
<div class="tab-content" id="mainTabContent">
  ...
  <div class="tab-pane fade show active" id="tab-standings" role="tabpanel">
    ...
    <div class="container-inner">
      ...
      <div class="scoring-rules-card mb-4">
        <div class="scoring-rules-inner">
          <div class="sr-section">
            <div class="sr-heading">⚽ Team Scoring</div>
            <div class="sr-row"><span class="sr-pts sr-win">+300</span><span class="sr-desc">Win (regulation, ET, or PK)</span></div>
            <div class="sr-row"><span class="sr-pts sr-draw">+100</span><span class="sr-desc">Regulation draw <em>or</em> ET/PK loss</span></div>
            <div class="sr-row"><span class="sr-pts sr-loss">+0</span><span class="sr-desc">Regulation loss</span></div>
          </div>
          <div class="sr-divider"></div>
          <div class="sr-section">
            <div class="sr-heading">🎯 Bonus Points</div>
            <div class="sr-row"><span class="sr-pts sr-bonus">+450</span><span class="sr-desc">Correct tournament winner</span></div>
            <div class="sr-row"><span class="sr-pts sr-bonus">+100</span><span class="sr-desc">Group stage advancement — Tiers A &amp; B</span></div>
            <div class="sr-row"><span class="sr-pts sr-bonus">+200</span><span class="sr-desc">Group stage advancement — Tiers C &amp; D</span></div>
            <div class="sr-row"><span class="sr-pts sr-bonus">+150</span><span class="sr-desc">Per goal scored by your Goal Scorer pick</span></div>
          </div>
        </div>
      </div>
      <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
