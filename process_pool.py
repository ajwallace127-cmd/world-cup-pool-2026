...
def _parse_scoring_plays(data: dict) -> dict:
    ...
    Three-pass strategy for maximum coverage:
      Pass 1 – structured scoringPlays / plays with participants array
      Pass 2 – text-based fallback for plays missing participants
      Pass 3 – boxscore player stats (goals column) if passes 1-2 found nothing
    """
    local: dict = defaultdict(int)
    ASSIST_ROLES = {"assist", "assister", "assisted by"}

    # Build scoring_plays list from whichever field ESPN populated
    ...
    # ── Pass 1: structured participants ──────────────────────────────────
    found_idx: set = set()
    for i, play in enumerate(scoring_plays):
        play_type = play.get("type", {}).get("text", "").lower()
        if "own" in play_type:
            found_idx.add(i)   # mark as handled so Pass 2 skips it
            continue
    ...
    # ESPN play text formats seen in practice:
    #   "Vinícius Júnior 54'"  |  "Vinícius Júnior (54')"  |  "Goal - Name"
    for i, play in enumerate(scoring_plays):
        if i in found_idx:
            continue
        play_type = play.get("type", {}).get("text", "").lower()
        ...
        if not text:
            continue
        # Strip trailing time markers like "45'", "(45'+2')", "90+3'"
        name = re.sub(r"[\s\(]*\d+\+?\d*'?\)?$", "", text).strip()
        # Strip leading "Goal -" / "Goal:" prefixes
        name = re.sub(r"(?i)^goal[\s\-:]+", "", name).strip()
        if name and 2 < len(name) < 60:
            local[_normalize(name)] += 1

    # ── Pass 3: boxscore player stats ────────────────────────────────────
    # Only use if passes 1-2 found nothing (data may double-count otherwise)
    if not local:
    ...
    def fetch_one(event_id: str) -> dict:
        try:
            ...
            if resp.status_code == 200:
                ...
                # ── DEBUG: print raw ESPN scorer data ──────────────────
                ...
                plays = [p for p in data.get("plays", []) if p.get("scoringPlay")]
                print(f"  [DEBUG {event_id}] {teams}")
                print(f"    top-level keys: {list(data.keys())}")
                print(f"    scoringPlays={len(sp)}  scoring plays in plays[]={len(plays)}  found={result}")
                # Print first scoring play in plays[] if any
                for p in plays[:3]:
                    print(f"    play: {p}")
                # Print boxscore player keys if present
                for tb in data.get("boxscore", {}).get("players", [])[:1]:
                    for sg in tb.get("statistics", [])[:1]:
                        print(f"    boxscore stat keys: {[s['name'] for s in sg.get('athletes',[{}])[0].get('stats',[]) if isinstance(s,dict)][:10]}")
                # Print keyEvents or timeline if present
                for key in ("keyEvents", "timeline", "incidents", "commentary"):
                    if data.get(key):
                        print(f"    {key}[0]: {data[key][0]}")
                        break
                # ── END DEBUG ──────────────────────────────────────────
                return result
...
def calculate_scores(entries: list, team_results: dict, advanced: set,
    ...
    for entry in entries:
        ...
        games_played = sum(d["wins"] + d["ties"] + d["losses"] for d in detail)
        standings.append({
...
def _build_bracket_tab(round_matches: dict) -> str:
    ...
    <div class="tab-pane fade" id="tab-bracket" role="tabpanel">
      ...
      <div class="container-inner">
        <p class="text-muted small mb-4">Fills in automatically as matches are played · Group stage runs June 11–July 1</p>
        {rounds_html}
      </div>
    </div>"""
...
def generate_html(standings: list, team_results: dict, advanced: set,
    ...
    for t in all_teams_data:
        ...
        if sg == "out" and prev_sg != "out":
            teams_rows += '<tr class="elim-divider"><td colspan="9">— Eliminated —</td></tr>'
        prev_sg = sg
        ...
        pts_str  = f'{t["pts"]:,}' if (t["pts"] or gp > 0) else '—'
          ...
          <table class="table mb-0" id="teams-table">
            <thead>
              <tr>
                ...
                <th class="text-center" onclick="sortTeams(5)">Advanced ↕</th>
                <th class="text-end"    onclick="sortTeams(6)">Pts ↕</th>
                <th class="text-center" onclick="sortTeams(7)">Picked ↕</th>
                <th class="text-end"    onclick="sortTeams(8)">% ↕</th>
              </tr>
            </thead>
