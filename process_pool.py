    ...
    def fetch_one(event_id: str) -> dict:
        try:
            ...
            if resp.status_code == 200:
                ...
                scoring_ke = [e for e in data.get("keyEvents", []) if e.get("scoringPlay")]
                print(f"  [DEBUG {event_id}] {teams}  →  found={result}")
                print(f"    scoringPlays={len(data.get('scoringPlays',[]))}  "
                      f"scoring_keyEvents={len(scoring_ke)}")
                for e in scoring_ke[:5]:
                    print(f"    keyEvent: {e}")
                # ── END DEBUG ──────────────────────────────────────────
                return result
