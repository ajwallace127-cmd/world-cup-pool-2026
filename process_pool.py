    ...
    def fetch_one(event_id: str) -> dict:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"  [DEBUG {event_id}] {teams} — {len(sp)} scoringPlays → found: {result}")
                for play in sp:
                    ptype = play.get("type", {}).get("text", "")
                    text  = play.get("text", "") or play.get("shortText", "")
                    parts = [(p.get("athlete", {}).get("displayName","?"),
                              p.get("type", {}).get("text","")) for p in play.get("participants", [])]
                    print(f"    type={ptype!r:20s} text={text!r:35s} participants={parts}")
                # ── END DEBUG ──────────────────────────────────────────
                return result
