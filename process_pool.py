...
TOURNAMENT_START = date(2026, 6, 12)   # First game kick-off date
...
MANUAL_ADVANCED: set = set()  # Award advancement bonus before knockouts begin
...
def _parse_scoring_plays(data: dict) -> dict:
    ...
    for play in scoring_plays:
        ...
        for participant in play.get("participants", []):
            ...
            if role in ("scorer", "goal scorer", "goalscorer", ""):
                name = participant.get("athlete", {}).get("displayName", "")
                if name:
                    local[_normalize(name)] += 1
                    break
    return dict(local)
    ...
    def fetch_one(event_id: str) -> dict:
        try:
            ...
            if resp.status_code == 200:
                return _parse_scoring_plays(resp.json())
        except Exception:
            pass
        return {}
...
def fetch_results() -> tuple[dict, set, set, set, dict, dict]:
    ...
    for team, count in group_game_count.items():
        if count >= 3 and team not in advanced:
            knocked_out.add(team)
    ...
    return dict(team_results), advanced, champion, knocked_out, scorer_goals, dict(round_matches)
...
def match_team(pick: str, team_results: dict, advanced: set,
    ...
    all_names = set(team_results.keys()) | advanced | champion | knocked_out
    ...
    for name in all_names:
        if _normalize(name) == pick_norm:
            ...
            return {
                ...
                "advanced": name in advanced, "champion": name in champion,
                "knocked_out": name in knocked_out, "matched_name": name,
                "has_played": r["wins"] + r["ties"] + r["losses"] > 0 or name in advanced,
                "pts_so_far": pts,
            }
    ...
    if best_name:
        ...
        return {
            "wins": r["wins"], "ties": r["ties"], "losses": r["losses"],
            "advanced": best_name in advanced, "champion": best_name in champion,
            "knocked_out": best_name in knocked_out, "matched_name": best_name,
            "has_played": r["wins"] + r["ties"] + r["losses"] > 0 or best_name in advanced,
            "pts_so_far": pts,
        }
    return _empty
...
def _build_group_tab(group_standings: dict) -> str:
    ...
    <div class="tab-pane fade" id="tab-groups" role="tabpanel">
      <div class="tab-hero" style="--tab-bg:url('images/groups_bg.jpg')">
        <div class="tab-hero-inner">
          <div class="tab-hero-title">Group Stage</div>
          <div class="tab-hero-sub">Standings will appear once the tournament kicks off (June 12)</div>
        </div>
      </div>
...
def _build_bracket_tab(round_matches: dict) -> str:
    ...
    <div class="tab-pane fade" id="tab-bracket" role="tabpanel">
      <div class="tab-hero" style="--tab-bg:url('images/bracket_bg.jpg')">
        <div class="tab-hero-inner">
      ...
      <div class="container-inner">
        <div class="empty-state">
          ⚽ <strong>Knockout rounds haven't started yet.</strong><br>
          The bracket will fill in automatically as matches are played.
        </div>
      </div>
    </div>"""
...
def _build_scorers_tab(standings: list, scorer_goals: dict, entries: list) -> str:
    ...
    <div class="tab-pane fade" id="tab-scorers" role="tabpanel">
      <div class="tab-hero" style="--tab-bg:url('images/scorers_bg.jpg')">
        <div class="tab-hero-inner">
    ...
    for rank, (name, data) in enumerate(sorted_scorers, 1):
        ...
        picker_str = ", ".join(data["pickers"][:4])
        if len(data["pickers"]) > 4:
            picker_str += f" +{len(data['pickers'])-4} more"
        ...
        <div class="sc-row {row_cls}">
          ...
          <div class="sc-info">
            ...
            <div class="sc-pickers">Picked by: {_h(picker_str)}</div>
            {goal_bar}
          </div>
          <div class="sc-stats">
            {goals_html}
      ...
      <div class="container-inner">
        <div class="sc-summary">
          <div class="sc-sum-card">
            <div class="sc-sum-val">{len(scorer_picks)}</div>
            <div class="sc-sum-lbl">Players Picked</div>
          </div>
          <div class="sc-sum-card">
            <div class="sc-sum-val">{total_goals}</div>
            <div class="sc-sum-lbl">Goals Scored</div>
          </div>
          <div class="sc-sum-card">
            <div class="sc-sum-val">{total_pts:,}</div>
            <div class="sc-sum-lbl">Total Pts Earned</div>
          </div>
        </div>
...
def _build_raw_data_tab(standings: list, team_results: dict,
    ...
    for s in standings:
        ...
        <tr class="rp-row" data-name="{_h(s['name'].lower())}">
          <td class="rp-rank-cell"><span class="rank-badge {rank_css}" style="width:22px;height:22px;font-size:.65rem">{rank}</span></td>
    ...
    <div class="tab-pane fade" id="tab-raw" role="tabpanel">
      <div class="tab-hero" style="--tab-bg:url('images/allpicks_bg.jpg')">
        <div class="tab-hero-inner">
...
def generate_html(standings: list, team_results: dict, advanced: set,
                  ...
                  round_matches: dict, fetch_attempted: bool = False) -> str:

    last_updated = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
    total_p      = len(standings)
    total_games  = sum(v["wins"] for v in team_results.values())
    ...
    stale = (
        ...
        if (fetch_attempted and active and not team_results) else ""
    )
    ...
    for tier, teams in TIER_DEFS.items():
        for team in teams:
            ...
            all_teams_data.append({
                ...
                "wins": wins, "ties": ties, "losses": losses, "pts": pts,
    ...
    for t in all_teams_data:
        ...
        if sg == "out" and prev_sg != "out":
            teams_rows += '<tr class="elim-divider"><td colspan="8">— Eliminated —</td></tr>'
        prev_sg = sg
        if t["status"] == "champion":
            row_cls, status_html = "tr-champ", '<span class="ts-champ">🏆 Champion</span>'
        elif t["status"] in ("advanced", "active"):
        ...
        <tr class="{row_cls}">
          ...
          <td class="text-center">{'✓' if t['advanced'] else '—'}</td>
          <td class="text-end fw-semibold">{pts_str}</td>
  ...
  <style>
    ...
    *, body {{ font-family:'Inter',sans-serif; color:var(--text); }}
    ...
    .hero {{
      position:relative;
      background:
        linear-gradient(to bottom, rgba(6,78,59,.82) 0%, rgba(6,95,70,.9) 60%, rgba(3,47,36,1) 100%),
    ...
    .hero::before {{
      ...
      background-image:
        radial-gradient(ellipse 60% 50% at 50% 100%, rgba(255,255,255,.06) 0%, transparent 70%),
        repeating-linear-gradient(90deg, transparent, transparent 9.09%, rgba(255,255,255,.025) 9.09%, rgba(255,255,255,.025) 9.2%);
      pointer-events:none;
    }}
    ...
    .hero-sub     {{ font-size:.9rem; opacity:.7; margin-top:8px; letter-spacing:.5px; }}
    .hero-updated {{ font-size:.72rem; opacity:.5; margin-top:5px; }}
    .stat-card {{
    ...
    .tab-hero {{
      height:110px;
      background:
        linear-gradient(to right, rgba(6,78,59,.88) 0%, rgba(6,78,59,.6) 50%, rgba(6,78,59,.85) 100%),
        var(--tab-bg, none) center/cover no-repeat;
    ...
    .rank-badge {{
      ...
      width:30px; height:30px; border-radius:6px; font-weight:800; font-size:.85rem;
    }}
    ...
    .rank-n {{ background:#ECFDF5; color:var(--green); border:1px solid #A7F3D0; font-size:.78rem; }}
    ...
    <ul class="nav nav-tabs" id="mainTabs" role="tablist">
      ...
      <li class="nav-item" role="presentation">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-scorers"
                type="button" role="tab">⚽ Scorers</button>
      </li>
      <li class="nav-item" role="presentation">
...
<div class="tab-content" id="mainTabContent">
  ...
  <div class="tab-pane fade show active" id="tab-standings" role="tabpanel">
    <div class="tab-hero" style="--tab-bg:url('images/standings_bg.jpg')">
      <div class="tab-hero-inner">
        <div class="tab-hero-title">🏆 Standings</div>
        <div class="tab-hero-sub">Click any row to expand picks · auto-updates every 2 hours</div>
      </div>
    </div>
    <div class="container-inner">
      {stale}
          ...
          <table class="table mb-0" id="teams-table">
            <thead>
              <tr>
                ...
                <th class="text-center" onclick="sortTeams(4)">Advanced ↕</th>
                <th class="text-end"    onclick="sortTeams(5)">Pts ↕</th>
...
if __name__ == "__main__":
    ...
    if no_fetch:
        ...
        team_results, advanced, champion, knocked_out, scorer_goals, round_matches = fetch_results()
